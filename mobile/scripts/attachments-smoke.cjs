...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '放弃草稿', exact: true }).last().click();
    await page.getByRole('button', { name: '修改', exact: true }).click();
    ...
    if (
      result.memories[0].attachments.length !== 1 ||
      result.memories[0].attachments[0].id !== 'p2'
    )
      throw Error('removed attachments not persisted correctly');
    console.log(
      'PASS: photo/audio removal, cancel preserves originals, save persists only retained attachments',
    );
  } finally {
    await browser.close();
  }
