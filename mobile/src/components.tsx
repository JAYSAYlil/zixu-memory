...
import type { Attachment, Memory } from './core';
...
export function AudioClip({ attachment }: { attachment: Attachment }) {
...
export function MemoryRow({ memory: m, onPress }: { memory: Memory; onPress: () => void }) {
  ...
  return (
    <ScalePressable
      accessibilityRole="button"
      accessibilityLabel={`打开记录：${m.text || '语音记录'}`}
      onPress={onPress}
      style={s.memory}
    >
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={s.root}>
        ...
        >
          <View style={[s.sheetShell, { backgroundColor: C.paper, borderTopLeftRadius: 28, borderTopRightRadius: 28, overflow: 'hidden' }]}>
            ...
            {!!feedback && <View accessibilityLiveRegion="polite" style={[s.feedback, { marginHorizontal: 20 }]}><Text style={s.small}>{feedback}</Text></View>}
          </View>
        </KeyboardAvoidingView>
      </SafeAreaView>
    </Modal>
...
export function Field({
  ...
  return (
    <View style={{ marginBottom: 17 }}>
      <Text style={[s.meta, { marginBottom: 8 }]}>{label}</Text>
      <TextInput
