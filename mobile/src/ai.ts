...
import { aiProfile, selfPolicy } from './self.ts';
export type RequestOptions = { signal?: AbortSignal; onStage?: (stage: string) => void };
  ...
  const answer = await completion(
    ...
    JSON.stringify({
      ...
      records: sources.map((m) => ({
        id: m.id,
        text: memoryText(m).slice(0, 3000),
        date: m.createdAt,
      })),
    }),
    options,
...
export async function completion(
  ...
  try {
    ...
    try {
      ...
      throw new Error('服务商响应格式不正确，请重试或检查接口兼容性。');
    }
    if (controller.signal.aborted) throw new DOMException('Aborted', 'AbortError');
    options.onStage?.('正在核对回答与来源…');
    ...
    if (e instanceof Error && e.name === 'AbortError')
      throw new Error(
        options.signal?.aborted
          ? '已取消请求，记录仍在本机。'
          : '请求超时。记录仍保存在本机，可以稍后重试。',
      );
    if (e instanceof TypeError) throw new Error('网络连接失败，请检查网络和服务商地址。');
    throw e;
  } finally {
    clearTimeout(timer);
...
export async function testConnection(config: ModelConfig, options: RequestOptions = {}) {
  return completion(config, '只回复：连接成功。', '测试连接。', options);
}
export async function observe(
  ...
  const raw = await completion(
    config,
    `${policy}\n从资料提出最多三条待验证的偏好、价值观或行为模式。每条至少引用两条不同记录。主动寻找反例，区分早期和近期经历，指出变化，不把一次情绪当成长期性格。尊重 userFeedback 中用户的改写，不重复已经确认、待确认或被拒绝的观察。证据不足返回 []。只输出 JSON 数组，不要 Markdown：[{"text":"具体且保留不确定性的观察", "category":"价值观", "sourceIds":["原始编号","原始编号"],"counterSourceIds":[],"changeNote":"时间变化或资料局限"}]。`,
    JSON.stringify({
      records: sources.map((m) => ({
        id: m.id,
        date: m.createdAt,
        text: memoryText(m).slice(0, 3000),
        excerpt: memoryText(m).length > 3000,
      })),
      userFeedback: insights.slice(0, 40).map((i) => ({
...
export async function recall(
  ...
  const answer = await completion(
    config,
    `${policy}\n回答问题，每个有关过去经历的陈述使用 [1] 这样的编号对应资料。资料不足而无法回答时，只回复“资料不足：”并简短解释缺少什么，不做经历推断。用户确认的档案也可能随时间改变。`,
    JSON.stringify({
      question,
      ...
      confirmedProfile: aiProfile(insights),
    }),
    options,
  );
  ...
  if (!/\[\d+\]/.test(answer) && !answer.trim().startsWith('资料不足：'))
    throw new Error('回答没有提供原文引用，因此没有展示。请重试或缩小问题范围。');
  return answer;
}
